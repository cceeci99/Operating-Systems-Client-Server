...
int main(int argc, char* argv[]){
    ...
    for ( int i=0; i < number_of_childs; i++ ) {
        ...
        if (pids[i] == 0) {  // CHILD PROCESS aka CLIENT
            srand(time(NULL) +
                  getpid()); // because children are created at the same time the seed will be same for all of them

            usleep(1); // sleep for millisecond so that the next client is random
...
//                total += run_time[j];
                // average_time = total/number_of_requests
